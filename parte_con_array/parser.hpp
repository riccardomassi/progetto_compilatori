...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    union union_type
    {
      ...
      // assignment
      char dummy8[sizeof (VarAssignAST*)];

      // binding
      char dummy9[sizeof (VarBindingAST*)];

      // globalvar
      char dummy10[sizeof (VarGlobalAST*)];
      ...
      // vardefs
      char dummy14[sizeof (std::vector<VarBindingAST*>)];
    ...
    struct symbol_kind
    {
      enum symbol_kind_type
      {
        ...
        S_startsymb = 33,                        // startsymb
        S_program = 34,                          // program
        S_top = 35,                              // top
        S_definition = 36,                       // definition
        S_external = 37,                         // external
        S_proto = 38,                            // proto
        S_globalvar = 39,                        // globalvar
        S_idseq = 40,                            // idseq
        S_stmts = 41,                            // stmts
        S_stmt = 42,                             // stmt
        S_ifstmt = 43,                           // ifstmt
        S_forstmt = 44,                          // forstmt
        S_init = 45,                             // init
        S_assignment = 46,                       // assignment
        S_block = 47,                            // block
        S_vardefs = 48,                          // vardefs
        S_binding = 49,                          // binding
        S_exp = 50,                              // exp
        S_initexp = 51,                          // initexp
        S_expif = 52,                            // expif
        S_condexp = 53,                          // condexp
        S_relexp = 54,                           // relexp
        S_idexp = 55,                            // idexp
        S_optexp = 56,                           // optexp
        S_explist = 57                           // explist
      };
    };
        ...
        switch (this->kind ())
    {
      ...
      case symbol_kind::S_assignment: // assignment
        value.move< VarAssignAST* > (std::move (that.value));
        break;

      case symbol_kind::S_binding: // binding
        value.move< VarBindingAST* > (std::move (that.value));
        break;

      case symbol_kind::S_globalvar: // globalvar
        value.move< VarGlobalAST* > (std::move (that.value));
        break;
      ...
      case symbol_kind::S_vardefs: // vardefs
        value.move< std::vector<VarBindingAST*> > (std::move (that.value));
        break;
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, BlockExprAST*&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const BlockExprAST*& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, VarAssignAST*&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const VarAssignAST*& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, VarBindingAST*&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const VarBindingAST*& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, VarGlobalAST*&& v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const VarGlobalAST*& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
      ...
      {}
#else
      basic_symbol (typename Base::kind_type t, const std::vector<VarBindingAST*>& v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
switch (yykind)
    {
      ...
      case symbol_kind::S_assignment: // assignment
        value.template destroy< VarAssignAST* > ();
        break;

      case symbol_kind::S_binding: // binding
        value.template destroy< VarBindingAST* > ();
        break;

      case symbol_kind::S_globalvar: // globalvar
        value.template destroy< VarGlobalAST* > ();
        break;
      ...
      case symbol_kind::S_vardefs: // vardefs
        value.template destroy< std::vector<VarBindingAST*> > ();
        break;
    ...
    , location (that.location)
  {
    switch (this->kind ())
    {
      ...
      case symbol_kind::S_assignment: // assignment
        value.copy< VarAssignAST* > (YY_MOVE (that.value));
        break;

      case symbol_kind::S_binding: // binding
        value.copy< VarBindingAST* > (YY_MOVE (that.value));
        break;

      case symbol_kind::S_globalvar: // globalvar
        value.copy< VarGlobalAST* > (YY_MOVE (that.value));
        break;
      ...
      case symbol_kind::S_vardefs: // vardefs
        value.copy< std::vector<VarBindingAST*> > (YY_MOVE (that.value));
        break;
  ...
  parser::basic_symbol<Base>::move (basic_symbol& s)
  {
    ...
    switch (this->kind ())
    {
      ...
      case symbol_kind::S_assignment: // assignment
        value.move< VarAssignAST* > (YY_MOVE (s.value));
        break;

      case symbol_kind::S_binding: // binding
        value.move< VarBindingAST* > (YY_MOVE (s.value));
        break;

      case symbol_kind::S_globalvar: // globalvar
        value.move< VarGlobalAST* > (YY_MOVE (s.value));
        break;
      ...
      case symbol_kind::S_vardefs: // vardefs
        value.move< std::vector<VarBindingAST*> > (YY_MOVE (s.value));
        break;
